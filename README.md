...
| [`/translate-content`](skills/translate-content/SKILL.md) | Translate prose — Play Store release notes, app descriptions, FAQs, marketing copy. Reads a per-repo config for output layout, target locales, and char limits. Handles cross-sentence consistency, tone, and whatever char limit the content type carries. |
...
| [`/post-merge-cleanup`](skills/post-merge-cleanup/SKILL.md) | Put a repo back to a clean state after a PR lands. Moves the session out of whatever worktree it was in and back to the main checkout, prunes remote-tracking refs, removes the worktrees and local branches whose remote branch is gone, prunes stale worktree entries, and fast-forwards the default branch. Refuses to delete anything holding uncommitted work — an untracked file in a worktree exists nowhere else — and asks before deleting a merged branch whose remote still exists. Cleanup only: it doesn't merge the PR, close issues, or tag anything. |
...
Once it merges, `/post-merge-cleanup` closes the loop the other way: it walks the session back out of the branch's worktree to the main checkout, prunes the refs, deletes the worktree and the local branch the merge made stale, and fast-forwards the default branch. It reads the merge off git rather than off GitHub — a branch whose remote branch is gone after `git fetch --prune` is one the merge deleted, which is also how it catches squash merges, since those leave no trace in `git branch --merged`. Anything it can't prove is merged, it reports and leaves. It never removes a worktree with uncommitted or untracked work in it, at all, under any flag.
