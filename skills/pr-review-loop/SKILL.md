...
**Reviewer roles.** Three sets, all bot lists: `request_on_pr_open` (requested on the first pass), `loop_reviewers` (re-requested on every push — these *drive convergence*), and `final_gate_reviewers` (requested once after convergence as a final adversarial pass). A bot in `request_on_pr_open` but **not** `loop_reviewers` is **first-pass-only**: it reviews once, its findings are evaluated like any other, but step 8 never re-requests it — so a rate-limited deep reviewer (e.g. Codex) contributes its first-look design/correctness catches without being dragged through every polish round. See `config/defaults.yml` for the recommended Codex pattern.
