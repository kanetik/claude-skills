...
**In that pass the courses are `rejected`, `acknowledged`, `deferred`, and `Ask-user`** — with `acknowledged` unavailable to any finding at a blocking severity, exactly as everywhere else (`reference/evaluation.md`). A terminating verdict normally carries no blocking findings at all, so the restriction rarely bites; where one is present it is not acknowledgeable just because the round is the last.
