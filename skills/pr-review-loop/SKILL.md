...
**Reviewer roles.** Four bot lists: `upfront_gate_reviewers` (gate before the loop — Phase 0), `request_on_pr_open` (requested on the first pass), `loop_reviewers` (re-requested on every push — these *drive convergence*), and `final_gate_reviewers` (requested once after convergence). A bot in `request_on_pr_open` but not `loop_reviewers` is **first-pass-only**: requested once, its findings triaged like any other, then never re-requested and not gating convergence. The recommended pattern for a rate-limited deep reviewer (Codex) puts it on the two gates while Copilot drives the loop — see `config/defaults.yml`.
...
- **Active set** (run-scoped): tracked minus bots already gone happy. This is who the loop still engages.

The loop's own fix push re-requests `active ∩ loop_reviewers` (step 8); first-pass-only and already-happy bots are skipped. Because the active set isn't recorded on the PR as such, a context-less wake reconstructs it from the carried wake payload — see `reference/waiting.md`.
...
1. **Wait `auto_review_grace_seconds`** from the baseline: iter 1 = `max(PR createdAt, latest push event)` (some auto-triggers fire on PR open even when the branch was pushed earlier); iter 2+ = the most recent push. Default `0` = no wait.
...
3. **For each bot, skip if it has already engaged the current commit** — started reviewing it or already delivered a verdict for it (evaluate at/after the most recent push across all three surfaces, plus the loop's own trigger). Copilot = a `reviewRequests` entry at/after the push. Codex = a `chatgpt-codex-connector` post OR an existing `@codex review` trigger at/after the push. Otherwise request it.
4. Proceed to step 3.

### 3. Wait for new reviewer activity
**Poll always; let events short-circuit.** Where a scheduling primitive exists, schedule a self-wake at `wait_check_cadence_seconds` as the baseline — *and*, if a PR-activity subscription also exists, subscribe so an event can wake you before the next tick. End the turn. On **any** wake — scheduled tick or event — re-pull and do a full three-surface reconciliation read against HEAD before concluding anything; events are a hint to look, never ground truth, and an event's *absence* never means a bot is still pending. With neither a scheduler nor a subscription, do one pass and hand back. Make every wake idempotent — reconstruct loop state from the PR plus the carried payload. Waiting does NOT count toward `max_iterations`. Full model, lockstep, re-entrancy, and carried state: `reference/waiting.md`.
...
**Loop terminates when all `loop_reviewers` are happy** — first-pass-only bots don't gate convergence. On termination, run the **final sanity check** (step 10) if `final_gate_reviewers` is non-empty, then the Final summary.
