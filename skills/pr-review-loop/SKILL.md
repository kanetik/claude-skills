...
**Staleness:** only signals at/after the most recent push count for the current HEAD. A clean verdict from before the latest push is stale — it does NOT make the bot happy for the new commit; re-derive against current HEAD. This re-derivation applies only to bots **still being tracked**. A bot that has *already* gone happy is dropped for good (step 4) — a later push does not pull it back in, even though its verdict now predates HEAD.
...
**Happy is terminal and sticky.** Once a bot goes happy (either path), it is **done for the rest of this loop run** — permanently removed from the tracked set. Do not re-request it, do not re-evaluate it, and **do not ask whether to review it again** on later iterations. This holds even when a subsequent commit/push moves HEAD: a later push (for *another* bot's fix) does NOT resurrect a dropped happy bot, even though its clean verdict is now technically stale against the new HEAD. The staleness/re-derive-against-HEAD rule (see "Reading reviewer state") governs bots that are **still tracked**; it never reaches back to a bot that has already gone happy. The **only** way a happy bot re-enters the loop is the user **manually** bringing it back (an explicit "re-run \<bot\>" / "have \<bot\> look again"). Absent that explicit instruction, a happy bot stays out — silently, no prompt. **Loop terminates when all tracked bots are happy** → Final summary report.
