...
- Its most recent communication *for the current HEAD* — taken across all three surfaces (the latest of: a formal review and its body, or a bot-authored issue comment) — reads as a clean verdict with no unaddressed concerns. Judge this from what the bot wrote per "Reading reviewer state" above, not a fixed phrase list.
