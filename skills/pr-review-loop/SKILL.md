...
description: >-
  ...
  upfront adversarial-gate review that runs to completion — the gate reviewer reviews,
  you apply its changes, and it re-reviews until it signs off — before the loop, then
  requests AI reviewers (Copilot, Codex, and any
...
An iterative loop that drives AI code reviewers (Copilot, Codex, any bot that posts) to convergence on a pull request: request → wait → evaluate → fix/answer → push → repeat, until every **loop reviewer** is happy. It can be bookended by two optional out-of-loop gates: an **adversarial upfront gate** (Phase 0) that vets the approach *before* the loop starts and runs the gate reviewer to completion (review → fix → re-review until it signs off), and a lighter **final sanity check** (step 10) by a configured bot after convergence. "Convergence" is gauged on `loop_reviewers`; out-of-loop reviewers are triaged but don't gate the loop — see "Reviewer roles" below.
...
- [`reference/evaluation.md`](reference/evaluation.md) — the step-5 lens/mindset rubric, courses of action, resolve criteria, and the Phase-0 upfront-gate triage (actionable-clear → fix and re-review to clean / actionable-unclear → ask the user).
...
**Reviewer roles.** Four sets, all bot lists: `upfront_gate_reviewers` (run as a gate *before* the loop — Phase 0), `request_on_pr_open` (requested on the first pass), `loop_reviewers` (re-requested on every push — these *drive convergence*), and `final_gate_reviewers` (requested once after convergence). Both gate sets are **out-of-loop** reviewers (outside `loop_reviewers`), but they differ in character: the upfront gate runs an *adversarial* review that completes before the loop — with its own triage (apply clear fixes, ask on unclear ones) and a re-review sub-loop that runs until the gate reviewer signs off on every change it prompted — while the final sanity check (step 10) is a lighter pass after it.

**Out-of-loop** is a **structural** property here, not a vibe: it means any tracked reviewer that sits **outside** `loop_reviewers`. The loop reviewers are the iterative *drivers*; every other tracked bot is out-of-loop — tracked and evaluated like any other, but not re-requested by step 8 across the loop's own fix pushes, so it isn't dragged through every polish round (the Phase-0 gate sub-loop and external pushes are the documented exceptions that re-engage it). The configured forms are **first-pass-only** bots (in `request_on_pr_open` but not `loop_reviewers`), `upfront_gate_reviewers`, and `final_gate_reviewers`; a bot that simply auto-appears unconfigured (tracked because it posted, but in no config list) lands in the same bucket. **Character is a separate axis from this structure:** the **upfront gate** is an *adversarial* review — its job is to find design/correctness problems before the loop — whereas the **final sanity check** is lighter, a last confirming look after convergence. A rate-limited deep reviewer (e.g. Codex) thus contributes its first-look design/correctness catches via the upfront gate without being dragged through every polish round. Being on the *PR-open* request is not itself out-of-loop — Copilot is on `request_on_pr_open` too and is a loop driver. See `config/defaults.yml` for the recommended Codex pattern.
...
But first, **only gate a *fresh* PR — never one that's already mid-flight.** The discriminator is prior *work on the PR*, not the mere presence of a comment: a brand-new PR that an auto-reviewer (Copilot, a Codex auto-review, a ruleset) has already commented on is still **fresh**, and the gate SHOULD run — that initial auto-review never counts as mid-flight. (This is the common case: bots that auto-review on PR open would otherwise suppress the gate on every fresh PR, which defeats its purpose.) Treat the PR as **mid-flight and skip Phase 0** — straight to iteration-1 branching — only when it shows real prior iteration: a `PR-Review-Loop: N≥1` iteration trailer (the loop proper already ran here — the gate's own `PR-Review-Loop: 0` fix commits do **not** count as mid-flight, so a gate re-review sub-loop fix never makes the gate skip itself), human (non-bot) review or reply participation, resolved review threads, or several iteration pushes/force-pushes. None of those present → it's fresh → run the gate. Skipping a mid-flight PR avoids surprise-injecting a design gate that would re-litigate settled ground.
...
3. **Read the verdict** across all three reviewer-state surfaces ("Reading reviewer state") and **triage it as one integrated judgement** (`reference/evaluation.md` → "Upfront gate triage") into exactly one outcome:
   - **Clean, or all findings deferred** (a clean verdict, or every finding resolved without a code change via `Create-issue-and-close` / `Reject-with-explanation`) → the gate is satisfied. Resolve any threads, then fall through to **iteration-1 branching**. (Deferred findings need no re-review — there's nothing new for the bot to see.)
   - **Actionable with a clear fix** — you're confident both that there's a real issue *and* what the correct change is. **Size is irrelevant**: a one-line rename and a structural redesign are the same outcome as long as you're sure of the path. → Apply it (evaluate per step 5, commit per step 6, update the description per step 7, push per step 8), then **re-request the same gate bot** and return to Phase 0 step 2. This is the gate's **re-review sub-loop**: review → apply → re-request, repeating until the gate bot itself posts a clean-or-deferred verdict on what you changed. It has no iteration cap (it precedes, and does not count toward, `max_iterations`). If it stops converging, or a re-review surfaces a finding you can't confidently resolve, fall to the `Ask-user` outcome rather than spinning.
   - **Actionable but unclear** (ambiguous, multiple viable approaches, security/auth/data-model/API-contract judgement, or anything you shouldn't decide unilaterally) → **pause and ask the user** (`Ask-user`). Do not proceed to the loop until it's resolved. The user's answer may convert it to a clear fix (which then re-reviews to clean as above) or a reject.

**The gate is not satisfied — and the loop must not start — until the gate reviewer's own latest verdict is clean or all-deferred at/after your final gate change.** A fresh fix the reviewer hasn't seen never passes the gate on its own: "complete the review" means the adversarial reviewer has reviewed, and signed off on, every change it prompted.

A verdict can carry several findings at once; judge them together. If **any** finding is actionable-but-unclear, take the `Ask-user` outcome (the most conservative) — you may still apply the unambiguously-clear fixes in the same push, but the gate is not satisfied until both the unclear question is settled *and* the gate reviewer has re-reviewed the result clean. Findings deferred without a code change (`Create-issue-and-close` / `Reject-with-explanation`) don't block the gate and need no re-review — they count toward the clean/deferred pass, matching `reference/evaluation.md`.
