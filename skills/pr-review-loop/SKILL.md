...
**Re-entrancy.** In the common single-context run Phase 0 just runs inline before the loop, so this is moot. A context-less wake reconstructs "still in Phase 0" primarily from the **carried payload** (note the flag, like the iteration counter — `reference/waiting.md`). Absent a carried flag, fall back conservatively: you're still in Phase 0 only if the PR is **fresh** by the mid-flight test above — a `PR-Review-Loop: N≥1` iteration trailer or human review participation means the loop proper already began, so it's *not* Phase 0; the gate's own `PR-Review-Loop: 0` fixes don't count as mid-flight — **and** `upfront_gate_reviewers` is set **and** not **all** gate bots are clean at/after current HEAD. Otherwise the gate has passed (or never applied): resume in the loop.
