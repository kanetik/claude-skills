...
**All-rejections short-circuit:** if a bot's latest comments ALL resolved to `Reject-with-explanation`, it's done — further iteration won't help. Any `Create-issue-and-close` (a deferred real concern = acceptance) disqualifies that call.
...
**The loop terminates when every reviewer in the active set is happy.** Then go to the final summary.
...
- **How it ended** — *converged* (every reviewer happy) or *cap reached* (say the count and what's outstanding). Different outcomes; say which.
