...
**Excusing the last reviewer is not convergence.** An excused reviewer is one that did *not* review, so where excusing it would leave `active ∩ reviewers` empty the invariant above fails and the run reports *nobody reviewed this HEAD*. Say that when you make the offer, so the user is choosing between a review and no review rather than appearing to pick between two ways of finishing.
...
**The convergence invariant.** Before reporting *converged*, both of these must hold, and neither is implied by the per-reviewer test below:

- **`active ∩ reviewers` is non-empty.** "Every reviewer is happy" is vacuously true of no reviewers, and every hatch on this page can empty the set — a reviewer excused, a modifier that narrowed the list, an unusable `skeptic` dropped at kickoff.
- **Every reviewer in it has verdicted the current HEAD.** Not a previous one. A fix push you have not had reviewed is the state this whole skill exists to prevent from being called done.

Fail either and it is **not** convergence, whatever the per-reviewer checks say. Report it as its own outcome: *nobody reviewed this HEAD*, naming the last commit and why the set is empty or unreviewed. This is the one mis-report that cannot be recovered from downstream, because a converged verdict is exactly what a reader stops checking.

**Terminal states, reported differently.**

1. **Converged** — the invariant holds and every reviewer in `active ∩ reviewers` is happy. Disposition and resolve any remaining non-blocking findings in the terminating verdict first (below), then go to the final summary.
...
**The terminating round still dispositions its own findings.** Terminal state 1 goes to the summary without passing through step 5, so the findings in the verdict that *ended* the loop — a clean pass carrying three `MEDIUM` nits, typically — would never get a reply or a resolved thread. That is the commonest terminating round there is, and leaving it undispositioned breaks the rule this skill leads with: every finding gets a decision. So before the summary, run step 5's recording half over them — reply, disposition marker, resolve — with no commit and no push. Skip it and the next run's cross-check buckets them `new`, which is the treadmill arriving through the one round nobody thought to disposition.
