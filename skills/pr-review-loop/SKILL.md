...
3. **Read the verdict** across all three reviewer-state surfaces ("Reading reviewer state") and **triage it as one integrated judgement** (`reference/evaluation.md` → "Upfront gate triage") into exactly one outcome:
   ...
   - **Actionable-clear** — you're confident both that there's a real issue *and* what the correct change is. **Size is irrelevant**: a one-line rename and a structural redesign are the same outcome as long as you're sure of the path. → Apply it (evaluate per step 5, commit per step 6, update the description per step 7, push per step 8), then **re-request the same gate bot(s)** and return to Phase 0 step 2. This is the gate's **re-review sub-loop**: review → apply → re-request, repeating until every configured gate bot posts a clean-or-all-deferred verdict on what you changed. It has no iteration cap (it precedes, and does not count toward, `max_iterations`). If it stops converging, or a re-review surfaces a finding you can't confidently resolve, fall to the `Ask-user` outcome rather than spinning.
...
**The gate is not satisfied — and the loop must not start — until *every* configured gate reviewer's own latest verdict is clean or all-deferred at/after your final gate change** (with several gate bots, the whole set must sign off, not just one). A fresh fix the reviewers haven't seen never passes the gate on its own: "complete the review" means the gate bot(s) have reviewed, and signed off on, every change they prompted.

A verdict can carry several findings at once; judge them together. If **any** finding is actionable-unclear, take the `Ask-user` outcome (the most conservative) — you may still apply the unambiguously-clear fixes in the same push, but the gate is not satisfied until both the unclear question is settled *and* the gate reviewer has re-reviewed the result clean. Findings deferred without a code change (`Create-issue-and-close` / `Reject-with-explanation`) don't block the gate and need no re-review — they count toward the "clean or all-deferred" pass, matching `reference/evaluation.md`.
