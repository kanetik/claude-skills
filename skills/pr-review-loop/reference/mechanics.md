...
Non-empty `project`, `users`, `irreplaceable_data`, `production_status`, `architecture` after the merge → good. Any still empty → check one more place before pausing: an **uncommitted copy in the PR repo's working tree**, which satisfies the skeptic skill whenever the file is not part of the change (`git -C "$REPO" cat-file -e "<head-sha>:.github/pr-review-skeptic.config.yml"` must *fail*). That is the file that skill's own stage 2 writes after interviewing a user, so a run started right afterwards would otherwise be told to add a config already sitting in the checkout with all five keys filled. Where it supplies them, proceed and say the config is uncommitted so it covers this run only. Still empty after that → pause. This check exists because an **agent-invoked** skeptic run does not interview for missing keys — it tears down and hands back what's missing. Discovering that inside the first round wastes a staging round and reads like a failure rather than a setup step.
