...
- **The current HEAD sha** — `gh pr view <num> --json headRefOid`. Every staleness question in this skill is answered against it (SKILL.md "Reading reviewer state"): a verdict counts when it was produced against *this* sha. Fetch it once per wake, before reading any reviewer state, and compare against it rather than against a clock.
- **Each review's sha.** For a bot, add `commit{oid}` to the `reviews` selection below — `PullRequestReview` exposes it, so the query the loop already runs can carry it and there is no need for a second paginated source that could disagree with the first. Skeptic's sha comes from its coverage record in the body instead (`<!-- pr-review-skeptic: reviewed=<sha> … -->`), which is the sha its reviewers actually read and is right on the force-push route where the review's own commit is not.
- **Whose review it is.** Select `author{login}` and **require the authenticated account** (`gh api user --jq .login`) before trusting a marker-carrying review's coverage record. On a public repo anyone can submit a review, and a body containing `<!-- pr-review-skeptic: reviewed=<current HEAD sha> unreviewed-units=0 -->` would otherwise make step 2 skip invoking skeptic, satisfy step 4's current-HEAD clause, and let the run report *converged* on a HEAD skeptic never read — the one mis-report SKILL.md says nothing downstream recovers from. `pr-review-skeptic` closes the same boundary for its own `$LASTREVIEWED`; the loop consumes the same PR-written value for a decision with the same blast radius. **Where the login cannot be determined** — `GET /user` 403s for a GitHub App installation token, including the `GITHUB_TOKEN` `gh` uses in Actions — say so, and treat skeptic as not having reviewed this HEAD **for this round**, so it is re-invoked rather than trusted. Do not treat it as a permanent bar: skeptic degrades to first-run scope in the same environment and still produces a full review, so the round proceeds. **Add this to the kickoff check** alongside the other three preconditions (SKILL.md Preconditions) — a run whose login is unreadable will re-invoke skeptic every round and can never converge on it, and that is the sort of thing the kickoff check exists to say at the start rather than at the cap.
- **Reviews with state, body, comment count, author type.** `__typename: Bot` identifies bot reviewers; **the body is not optional here**, because a review authored by a `User` carrying the marker line `<!-- pr-review-skeptic -->` is the skeptic reviewer, not a human. Classify on the marker first, author type second — a query that reads `__typename` and drops `body` cannot tell them apart, and the loop then skips its own reviewer as human participation:
  `reviews(last:100){nodes{author{login __typename} state body submittedAt commit{oid} comments{totalCount}}}`
...
- **Most recent push timestamp** — latest of `HeadRefPushedEvent.createdAt` / `HeadRefForcePushedEvent.createdAt` from the timeline (force-pushes count). NOT `committedDate`. This is the **fallback** staleness input, for a verdict that carries no sha (a bare issue comment); it is not the primary test, and it is legitimately **absent** on a PR opened from an already-pushed branch — the `gh pr create` flow. Where it is missing, the iteration-1 baseline (`createdAt`) stands in.
...
| A verdict whose coverage line says **no new code was in scope** | Also normal, and not a failure. The skeptic's content reviewers had an empty delta — usually because the base branch was merged into the PR branch between rounds, which moves the merge-base without adding PR code. Its composition reviewer still read the whole change against the new base, so the verdict is a real one. Triage it like any other; do not read it as a config or target problem. |
...
| No unit reviewed — no reviewer's blocks reached the run | **Not a clean round.** Report what the skill reported: the blocks did not arrive. It hits every unit at once, but that says nothing about the cause — the subagent tool being unavailable, rate-limited or erroring produces this, and so does a dispatch whose output had nowhere to go, which the skill's own stage 4 names first. Say so, and be exact about what a retry buys here: **changing the dispatch is not something this loop can do.** Step 2 hands skeptic the PR reference and nothing else, and the skill's own stage 4 has already spent its one changed-dispatch attempt internally before returning — so a retry from here re-runs the identical thing and spends another full set of reviewers producing reviews that go nowhere. Offer it only if the user has changed something themselves. Otherwise the live choices are use a bot, or proceed without it. "Proceed without it" means **excused for the run** (SKILL.md step 4) — drop it from `active`. Leaving it in place is not proceeding: with no verdict against the current HEAD sha and no stickiness to drop it, it can never be happy, so the loop runs to the cap over a reviewer that never ran. Where it is the only reviewer, say that proceeding means nothing will have reviewed this HEAD, so the run cannot come back converged. |
| **"Could not stage — another run holds the staging lock"**, with a path and a remaining wait | **Not the row above, and the difference matters.** Nothing was dispatched and nothing failed: a concurrent or crashed skeptic run owns the staging directory, and the skill refused to clear it rather than delete a worktree another run's reviewers may be reading. **Do not excuse the reviewer** — that trades a lock measured in minutes for a whole run with nothing independently reviewing it, and excusing the last reviewer is the *nobody reviewed this HEAD* outcome (SKILL.md step 4). Do not count the round toward `max_iterations` either: no review was solicited, so nothing was spent. Surface the lock path and the wait to the user verbatim and **pause**. The lock does expire on its own inside the window it prints — say so, because the alternative is a user deleting the lock of a run that is mid-pass, which sweeps the worktree that run's reviewers are reading. What it does not do is expire on *this loop's* timescale: rounds burn in seconds against a window measured in minutes, and because this round is not counted they never run out either — a timer-driven retry spins without terminating. That is why the answer is to pause rather than to re-invoke. Waiting it out or deleting the file are both the user's call, and both need the number the skill printed — which is a remaining wait on the deadline branch and a `<=1800s` upper bound on the other. |
