...
| Take a converged PR out of draft | `gh pr ready <num>`, guarded on `gh pr view <num> --json isDraft` (SKILL.md step 4 — terminal state 1 only, orchestrator-repo PRs only, and only where `mark_ready_on_convergence` is on) | — | — |
...
- **Reviews with state, body, comment count, author type.** `__typename: Bot` identifies bot reviewers; **the body is not optional here**, because a review authored by a `User` carrying the marker line `<!-- pr-review-skeptic -->` is the skeptic reviewer, not a human. Classify on the marker first, author type second — a query that reads `__typename` and drops `body` cannot tell them apart, and the loop then skips its own reviewer as human participation:
  `reviews(last:100){nodes{databaseId author{login __typename} state body submittedAt commit{oid} comments{totalCount}}}`
...
| A verdict whose coverage line says **no new code was in scope** | Also normal, and not a failure. The skeptic's content reviewers had an empty delta, and there are two causes. **Usually it is step 4's whole-change ask**, which deliberately re-invokes skeptic at an unmoved HEAD on a converging PR — the base has not moved, and the composition reviewer read the whole change against the same base. Otherwise the base branch was merged into the PR branch between rounds, which moves the merge-base without adding PR code, and the composition reviewer read the whole change against the *new* base. Either way the verdict is a real one. The ask is identifiable: it fires only when the prior record's `reviewed` already names current HEAD, which is on the PR, and it also rides in the wake payload. Where neither says so, the base moved — **or** the round's edits left the PR's own diff, since the content list is the delta *intersected with* the PR's file list, and a round that only reverts earlier PR changes back to base content empties that intersection with HEAD moved and the base still. Triage it like any other; do not read it as a config or target problem. |
