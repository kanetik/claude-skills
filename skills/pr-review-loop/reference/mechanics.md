...
- **Each review's `commit_id`** — the sha a bot reviewed. GraphQL's `reviews` connection does **not** expose it; the REST representation does, so where you need it take `gh api repos/{o}/{r}/pulls/{n}/reviews --paginate --jq '.[] | {id, commit_id, body, user: .user.login}'`. Skeptic's sha comes from its coverage record in the body instead (`<!-- pr-review-skeptic: reviewed=<sha> … -->`), which is the sha its reviewers actually read and is right on the force-push route where `commit_id` is not.
