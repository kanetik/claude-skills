...
| Get bot-authored PR issue comments (verdict surface) | Paginated GraphQL `comments` connection (see `reference/graphql.md`) — it carries `author.__typename` (Bot vs User) and numeric `databaseId`, both of which the loop needs | github MCP `get_pull_request` | `gh pr view <num> --json comments` as a quick **non-authoritative** read only — it doesn't paginate and its JSON omits `__typename` and the numeric `databaseId`, so don't derive bot state from it |
