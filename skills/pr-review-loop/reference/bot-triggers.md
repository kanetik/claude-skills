...
If a shell mangles the body for any reason, fall back to `--body-file` (write `@codex review` to a temp file, then `gh pr comment <num> --body-file <path>`, and delete the file) or the REST endpoint (`gh api repos/{o}/{r}/issues/{n}/comments -X POST -f body="@codex review"`).
