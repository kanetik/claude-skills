...
**Read the scope off the finding, and where you cannot, say so rather than guessing.** A skeptic comment from a mixed-scope run carries a line just above its marker — `<!-- pr-review-skeptic: scope=whole -->` or `scope=delta` — so filter on `scope=whole` and compute over those. It is a separate line from the plain `<!-- pr-review-skeptic -->` marker, which is unchanged and is still what identifies the comment. Three cases where there is no number to report, and all three are reported as *not attributable* rather than as a share: the marker carries no scope attribute (an older run, or one whose reviewers all had the same range — on a first run that means every finding counts); a reviewer's findings cannot be traced to a range at all; or no reviewer had the whole change in range. **Never fall back to the all-findings share** — that is the delta-contaminated number this rule exists to prevent, and reporting it under the words "computed over whole-change reviewers" is worse than reporting nothing. Where most of *those* findings are `repair`, the loop is reviewing itself, and that belongs in the run's report (SKILL.md step 9) — say it plainly. A finding that exists because this loop wrote the code it is about is worth naming as such, both to the user and on the thread where it helps.
