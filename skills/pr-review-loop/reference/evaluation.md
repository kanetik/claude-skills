...
- **In a *terminating* round the only courses are `rejected` and `deferred`** (SKILL.md step 4). That pass pushes nothing, so "take the cheap correct ones now" does not apply to it: a non-blocking finding worth fixing makes the round non-terminal instead, and it goes through the normal fix→push→re-review path so the reviewer sees the result.
