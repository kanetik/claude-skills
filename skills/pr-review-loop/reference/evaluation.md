...
**Give the checker the diff and the invariants. Nothing else.** Everything the round has done since the last push — `git add -N .`, then `git diff @{upstream}` — plus the one-line invariant you named for each fix. **Both halves are load-bearing and both fail silently.** Diff against the **pushed ref**, not against `HEAD`: this check guards the push, and a round's work does not always sit in the working tree. `git revert` commits by default, so after the revert lever above takes out a repair chain, `HEAD` *is* the round's work and `git diff HEAD` comes back empty — the checker would be handed nothing for the largest single act a round can perform. `@{upstream}` is right whether the round committed or not. And `add -N` is what puts a **newly created** file in range: no diff form shows an untracked one, so a fix that adds a reference doc or a config default is otherwise invisible. A checker handed a diff missing the round's work reports nothing wrong with it, and the round pushes under a clean pre-push read — worse than having run no check at all. **Not** the finding text, not the reviewer's rationale, not the PR description, not which round this is, not why you chose this fix over the one that was suggested. Withholding those is the entire mechanism: a checker told what the diff was *meant* to do reads the diff for confirmation, and confirms. This is the same discipline `pr-review-skeptic` applies to its own reviewers, applied here to a diff that skill will not see until after it ships.
