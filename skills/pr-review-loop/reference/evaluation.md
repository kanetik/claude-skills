...
- **Actionable-clear.** You're confident both that there's a real issue *and* what the correct change is — some `Fix-*` course you'd stand behind. **Size is irrelevant:** a one-line rename and a structural `Fix-broader` redesign are the same outcome as long as you're sure of the path. → Apply, re-request the gate bot, and **repeat to clean** — the gate's re-review sub-loop runs until the bot signs off on what you changed.
