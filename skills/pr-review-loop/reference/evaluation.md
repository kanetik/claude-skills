...
**Two inputs decide this question, and neither of them is in the finding.** A reviewer's text says what is wrong with the code. It cannot say whether this project cares, or how late in the run you are — and those are the two things the trade above is made against. Both are cheap and both are already to hand. Without them everything above is judged by a reader whose default disposition is to fix, and the failure is not theoretical: on the run these paragraphs were written from, 106 of 115 findings were fixed, including 51 of 52 `MEDIUM`s and 46 of 54 `LOW`s, with every rule on this page already in force.

**What this project counts as worth it.** The reviewed repo has already committed a description of itself — the five project keys in its `.github/pr-review-skeptic.config.yml`: what the application is, who uses it and roughly how many, what data cannot be recovered, whether it is shipping, and the shape of it. SKILL.md's Preconditions already require all five to hold a value before a run starts, so they are there on every round, in a file you have already opened. They are what makes *who hits this* answerable at all: "nobody without an unusual combination of settings" is a different answer for an internal tool than for an app in front of strangers, and the finding reads the same either way. Where the project has also written a `fix_bar` (`reference/configuration.md`), that is its own sentence about what a fix has to buy, and it governs this question directly.
