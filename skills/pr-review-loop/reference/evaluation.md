...
- **The base is the last pushed commit, never `HEAD`.** This check guards the push, and the round's work does not always sit uncommitted: `git revert` commits by default, so once the revert lever above takes out a repair chain, that removal *is* `HEAD` and a diff against `HEAD` comes back empty — nothing at all for the largest single act a round can perform.
...
**What comes back is yours to triage, in-round.** Real problem in your own fix → fix it now, before anything is recorded or committed, so nothing ships that you already know is wrong and no reply goes out describing a fix you are about to change. Not real → drop it. **Nothing from this check is posted, gets a thread, or gets a disposition marker.** It is not a reviewer and it produces no findings in the sense the rest of this skill uses the word — it is one more thing you did before committing, and the record of it is the fix itself.
