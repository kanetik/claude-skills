...
Half of what is needed is **already merged and already required on the runs that have it**, so this key does not ask for it again. Skeptic's config carries five project keys — `project`, `users`, `irreplaceable_data`, `production_status`, `architecture` — and SKILL.md's Preconditions refuse to start a **`skeptic`** run until all five hold a value. Step 5 merges them the way `reference/mechanics.md` sets out — that skill's own layers, ending at the reviewed repo's file — rather than reading any one of them, since the repo's file alone gets a different answer wherever a lower layer supplied a key. It is the same merge the kickoff check performs, and cheap enough to redo, so nothing about it rides in the wake payload. They answer *who hits this and what it costs them*, which is the half of question 2 a finding's own text cannot supply.
