...
Part of what is needed is **already merged and already required on the runs that have it**, so this key does not ask for it again. Skeptic's config carries five project keys — `project`, `users`, `irreplaceable_data`, `production_status`, `architecture` — and SKILL.md's Preconditions refuse to start a **`skeptic`** run until all five hold a value. Step 5 merges them the way `reference/mechanics.md` sets out — that skill's own layers, ending at the reviewed repo's file — rather than reading any one of them, since the repo's file alone gets a different answer wherever a lower layer supplied a key. It is the same merge the kickoff check performs, and cheap enough to redo, so nothing about it rides in the wake payload. They answer *who hits this and what it costs them* — one of the three things question 2 needs and a finding's own text cannot supply, alongside this key and step 9's numbers for the round.

**That precondition fires only where `skeptic` is in `reviewers`, and the shipped default is `[copilot]`.** A bots-only run has never required, checked or read that file, and may be running in a repo with none — so step 5 asks question 2 on `fix_bar` and the finding's own reach, exactly as it did before these keys were wired in, and does not go looking. It must not answer them itself either: Preconditions refuses to guess project facts on the user's behalf because five invented answers steer the reviewer, and five invented answers steer the author's triage the same way.
