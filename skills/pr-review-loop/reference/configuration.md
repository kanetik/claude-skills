...
| `final_gate_reviewers` | `[skeptic]` | Reviewers run **once** after convergence, as a final check (SKILL.md step 10) on what the fix rounds produced. Non-deferred findings → fix, re-converge `loop_reviewers`, re-run once, then hand back. `[]` = no check. Usually the same reviewer as the upfront gate. |
