...
| `whole_change_taper` | `2` | How many **consecutive whole-change reads with no blocking finding** satisfy step 4's third convergence condition when the last of them sits behind HEAD. Counts reads, not rounds. A read counts only while it is still on the branch and everything after it is the loop's own, so a blocking read, an external push and a history rewrite all re-arm it. See below. |
...
- **But it removes the *stop*, not the *report*.** Lifting the cap also removes the run's only scheduled moment of accountability, and that is not what anyone asks for when they say "just go until you're done". SKILL.md step 9 therefore re-reports the same evidence and re-asks every `max_iterations` rounds on an uncapped run. Saying "keep going" each time is a fine answer; never being asked is not one.

### `whole_change_taper`, and the condition that otherwise has no fixed point

Step 4's third convergence condition asks that the whole change has been read cold at the **current** HEAD. Every fix moves HEAD. So on a change large enough that a cold read finds something real every time, each fix owes another whole-change read, that read finds something, fixing it moves HEAD again, and the condition is never satisfied by anything the loop can do.

That is not hypothetical and it is not a reviewer fault. Measured on the run this key exists because of: four consecutive whole-change reads returned one `MEDIUM` and two `LOW` each, none of them wrong, across fifteen rounds in which nothing blocking was found at all. Three of the rounds between them dispositioned every finding without changing a line — terminal rounds by the rule this skill leads with — and none could terminate, because the sha was behind. The findings were a steady *rate*, and the condition was written as though they were a queue.

The taper answers that and nothing else: **N consecutive whole-change reads with no blocking finding satisfy the condition even when the last sits behind HEAD — for as long as everything behind HEAD is the loop's own repair work.** Counting reads rather than rounds is deliberate — the reads are what the guarantee is about, and the rounds between them are delta-scoped by construction. A blocking finding in a whole-change read resets the count, because that read is the cadence earning its cost and the clean reads have to come after it.

**Read what it does and does not loosen.** About the thing the condition guards — a cold read of the whole change before merge — it is *stricter* than the equality: two such reads rather than one. It is looser only about *when* — and about *how far*. Everything in the gap is the loop's own repair, each piece delta-reviewed before anything converges, but **no amount of it re-arms the taper**, blocking fixes included, so a long repair tail can sit behind a met taper never read as one thing. That is the cost this key buys; the converged summary names the sha so a reader can see how far back it is. That is why an **external push re-arms the taper**: a commit the loop did not write has had none of that, so the gap would no longer be bounded by anything. `reference/waiting.md` gives the test, including the ancestry guard a force-push needs. Those rounds converge; only the whole-change re-read does not. The converged summary names the sha the whole change was last read at, so a merger can see the gap rather than infer it.

`2` is the default. Raise it to buy more cold reads on a change you want held harder.

**A taper below `1` is a configuration error, not a mode — and it is checked rather than asserted.** At `0`, step 4's clause reads "the clause also holds once 0 consecutive whole-change reads have produced no blocking finding", which every state satisfies: the third condition passes for ever, the loop converges on the first happy round with a `whole-change` of `none`, and the summary has no sha to name. That is the guarantee this clause *is*, gone, and it is the one mis-report nothing downstream recovers from. So the merged value is checked the way the non-empty `reviewers` requirement is checked — as an **invariant, tested twice**: once before step 2, and again at every step-4 evaluation, since a scheduled wake resumes at step 3 and never re-runs the preamble. **Below `1` stops the run**, naming the layer that set it. Prose asserting a value cannot happen does not stop it happening — that is exactly why `reviewers: []` has its own section above rather than a sentence saying it would be silly. **No value switches the requirement off** — a taper of zero would mean converging with no whole-change read at all, which is the hole the condition was written to close. A PR small enough for the reviewer to run dry reaches the equality on its own, long before this key is in play.
