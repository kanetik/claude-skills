...
| Key | Default | Meaning |
|---|---|---|
| `upfront_gate_reviewers` | `[]` | Adversarial bots run as a gate **before** the loop (SKILL.md Phase 0). The review runs to **completion**: an **actionable-clear** finding (any size) → fix and re-request the same bot, repeating **to clean with no cap**; an **actionable-unclear** finding → pause and ask the user. The gate isn't satisfied until the bot's own verdict is clean or all-deferred at/after the final gate change, so the adversarial reviewer signs off on every change it prompted before the loop starts. `[]` = no upfront gate (loop starts immediately). Runs only on a **fresh** PR (per Phase 0's fresh/mid-flight test) — an initial bot auto-review on PR open does **not** suppress it; it's skipped only on a genuinely mid-flight PR. Typically a rate-limited deep reviewer, so a wrong-from-the-start approach is caught before Copilot polishes it. |
