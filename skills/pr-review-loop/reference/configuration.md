...
Part of what is needed is **already merged and already required on the runs that have it**, so this key does not ask for it again. Skeptic's config carries five project keys — `project`, `users`, `irreplaceable_data`, `production_status`, `architecture` — and SKILL.md's Preconditions refuse to start a **`skeptic`** run until all five hold a value. Step 5 merges them the way `reference/mechanics.md` sets out — all of it, including the last-resort source that file names — rather than reading any one layer, since the repo's file alone gets a different answer wherever another layer supplied a key. It is the same merge the kickoff check performs, and cheap enough to redo, so nothing about it rides in the wake payload. They answer *who hits this and what it costs them* — one of the three things question 2 needs and a finding's own text cannot supply, alongside this key and step 9's numbers for the round.

**That precondition fires only where `skeptic` is in `reviewers`, and the shipped default is `[copilot]` — which makes the five *unguaranteed*, not off-limits.** A bots-only run has never required or checked that file, and may be in a repo with none; it may equally be in a repo that committed one, which is the setup this skill's own documentation asks for. So step 5 runs the merge whatever the reviewer list, and uses whichever keys come back filled. Empty after the merge, it asks question 2 on `fix_bar` and the finding's own reach, exactly as it did before these keys were wired in.

**On no list does it answer them itself.** Preconditions refuses to guess project facts on the user's behalf because five invented answers steer the reviewer, and five invented answers steer the author's triage the same way. Not-guaranteed licenses a read; it never licenses an invention.
...
**What it does not mean is that the five are guaranteed to describe the reviewed project.** The reviewed repo's file wins where it fills a key; a user-level `~/.claude/pr-review-skeptic.config.yml` fills whatever it leaves empty, which is a legitimate setup — and on a cross-repo PR that layer describes the caller's usual project rather than the one under review. So a run can calibrate reach on a mixture. It is worth a sentence in the disposition where it shows, and it is not something this key can fix.
