...
Wait until every bot **requested for the current commit** has either delivered a verdict — a formal review OR a bot-authored issue comment (clean or findings) — OR been dropped as "happy" (step 4). The requested set depends on the round: iter 1 = `request_on_pr_open` (so first-pass-only bots like an adversarial Codex *are* waited for and batched on the first pass); the loop's own fix pushes = the re-requested `loop_reviewers` (first-pass-only bots aren't re-requested, so don't wait on them then); an external push = the full tracked set (per "External pushes during the wait"); the final adversarial gate = `final_gate_reviewers`. **Convergence/termination, however, is gauged on `loop_reviewers` only** (SKILL.md step 4) — a first-pass-only bot's verdict is triaged but never blocks the loop. **Batch-evaluate the combined feedback in one pass** — don't react to one bot's comments, push a fix, then let another bot review the new state. That compounds iteration count: bot B re-reviews your fresh diff and raises tangential comments that would've been weighed differently with both takes side by side. Seeing the round's bots together also surfaces contradictions (one says X, another ¬X) and overlap (both flag the same issue) before you decide.

**Newly appearing bots count.** If a bot posts a review — or a review-style verdict comment (findings or a clean pass; not CI/noise) — during the wait that wasn't previously tracked (e.g. an auto-triggering bot whose verdict lands after the grace window), add it to the tracked set immediately and batch its verdict into this round's lockstep (so you evaluate it alongside the others). But it joins as an **adversarial** bot — it's not in `loop_reviewers` unless configured there, so step 8 won't re-request it on later pushes and it doesn't gate convergence (which is gauged on `loop_reviewers`). Its findings are still triaged in step 5 like any other.
