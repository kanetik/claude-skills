...
The wait is **not** an either/or between polling and events. Polling is the baseline; events are an accelerator layered on top:

1. **Schedule a self-wake at `wait_check_cadence_seconds`** (default 240s) whenever a scheduling primitive exists (`/loop <cadence>`, `ScheduleWakeup`, `send_later`, `CronCreate`). This poll is the reliable floor — it fires whether or not any event ever arrives.
2. **Also subscribe to PR activity if a subscription primitive exists** (e.g. `subscribe_pr_activity` on GitHub-integrated web). An event then wakes you *before* the next scheduled tick — it short-circuits the current poll interval so you react sooner, not so you skip polling.
3. End the turn. On **any** wake — scheduled tick or event — re-pull and reconcile (below), then act.
4. **If neither primitive is detectable**, fall to the floor: do one review pass, then stop with "re-invoke `/pr-review-loop` to continue." Never busy-wait with `sleep` for external events.
