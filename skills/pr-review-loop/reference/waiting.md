...
Wait until every bot **engaged for the current commit** has either delivered a verdict (a formal review OR an issue comment, clean or findings), been dropped as happy, or been **excused for the run** (all three: step 4). The engaged set is `active ∩ reviewers` for a normal round, and **`tracked ∩ reviewers`** after an external push — previously-happy configured reviewers come back, uninvited ones never do.
