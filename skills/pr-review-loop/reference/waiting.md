...
**The whole-change taper is not in this list, because it is PR-derivable and should be re-derived rather than carried** (SKILL.md step 4). It counts consecutive whole-change reads with no blocking finding, and every one of those reads is a skeptic review on the PR: `scope=whole` above its comments, a verdict in its body, and a coverage record naming the sha. Walk them newest-first and stop at the first with a blocking finding — that is the count. A carried number would be a second copy of something the PR already answers exactly, and the failure modes are asymmetric: a lost count re-reads the whole change once more, which is the cost the taper is trading against, while a *wrong* carried count converges a PR on a cold read that never happened.
