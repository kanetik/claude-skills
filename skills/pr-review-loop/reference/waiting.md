...
Which bots have gone happy (SKILL.md step 4) is run-scoped state the PR doesn't record *as such*: a dropped bot's clean verdict still sits on the PR, and the PR-derived **tracked set** (step 2) still counts it as a member. A context-less wake that re-derives membership from PR history alone would therefore re-add it, see its now-stale clean verdict, and re-request it — the resurrection "Happy is terminal and sticky" forbids. The fix is to reconstruct the run-scoped **active set** (tracked minus already-happy), two ways, mirroring the iteration counter:

- **Primary — carry the dropped-happy set in the wake/continuation payload**, alongside the counter and modifiers, e.g. `"...; dropped happy: copilot, codex."` Those bots resume excluded from the active set.
- **Fallback — derive it from the PR.** A bot is dropped-happy when its most-recent verdict (across the three surfaces, see SKILL.md "Reading reviewer state") reads **clean** AND it has **not** been re-requested at/after the current HEAD — no `reviewRequests` entry or trigger comment for it after the latest push (the same per-bot "engaged the current commit" signal step 2.3 reads). The loop never re-requests a dropped bot, so a clean-verdict bot with no fresh trigger was deliberately dropped — keep it out. This also makes the **manual bring-back** self-deriving: a user re-requesting a previously-happy bot *is* a fresh trigger at/after HEAD, so the derivation returns it to the active set with no extra bookkeeping. (The fallback can only under-exclude in one harmless direction — if the user re-requested, the bot correctly comes back; it never wrongly resurrects a bot the user left alone.)
