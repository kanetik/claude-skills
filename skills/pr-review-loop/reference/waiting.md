...
**Still in Phase 0?** Carry a flag. Absent it, fall back — and note the fallback can only test the **bot** entries of `upfront_gate_reviewers`, since they're the ones with PR state to read; a `skeptic` entry is handled in the paragraph below. You're still in the upfront gate (SKILL.md Phase 0) only if `upfront_gate_reviewers` is non-empty, not **all** of its gate bots are clean at/after HEAD, and the loop proper hasn't begun — judged by the durable signals the gate's own sub-loop can't manufacture (`PR-Review-Loop: N≥1` trailer or human participation), **not** resolved threads / `: 0` commits (the gate sub-loop creates those itself). A mid-flight PR is never re-gated. Otherwise the gate has passed or never applied — resume in the loop.

**Past the loop entirely?** A `PR-Review-Loop-Phase: final` trailer on the newest loop commit (SKILL.md steps 6 and 10) says convergence already happened and the run is in its final check. A wake that finds one **does not re-enter the loop**, whatever the reviewer state at HEAD looks like — a final-check fix push deliberately leaves loop reviewers un-re-requested, so "a loop reviewer hasn't verdicted this HEAD" is the expected state there, not a reason to start iterating again. Resume in step 10: re-run the check once if it hasn't been, else summarize. This is the one trailer that changes which phase you're in rather than how far along you are.

**A `skeptic` gate leaves nothing on the PR to reconstruct from** — no review, no threads, no comment — so "not all gate reviewers are clean at/after HEAD" is unanswerable for it from PR state alone. That's tolerable because the local gate is **re-runnable**: where the fallback above lands you in Phase 0 and you can't tell whether the skeptic pass already cleared, just run it again. It re-reviews HEAD, costs a staging round, and returns clean if the gate was in fact satisfied. Re-running a cheap idempotent review beats resuming past a gate that never passed. Both the skeptic findings themselves and the verdict are turn-local, so where the gate is mid-sub-loop the wake payload should carry them (`"Phase 0, skeptic gate: 2 blocking findings outstanding — <one line each>"`); losing them costs a re-run, not correctness.

## Lockstep across the round's requested bots

Wait until every bot **requested for the current commit** has either delivered a verdict (a formal review OR an issue comment, clean or findings) OR been dropped as happy (step 4). The requested set depends on the round: upfront gate = the **bot** entries of `upfront_gate_reviewers` (each re-review in the gate sub-loop waits on the same set again — the "dropped as happy" escape is a loop mechanism that does **not** apply inside the gate, which isn't satisfied until every gate reviewer is clean/all-deferred at/after the final gate change); iter 1 = `request_on_pr_open` (first-pass-only bots *are* waited for and batched on the first pass); the loop's own fix push = the re-requested `loop_reviewers` (don't wait on first-pass-only bots then); an external push = the full tracked set; the final check = the **bot** entries of `final_gate_reviewers`. **Convergence is gauged on `loop_reviewers` only** — a first-pass-only bot's verdict is triaged but never blocks the loop.

**A `skeptic` gate entry is never waited on.** It isn't requested, so nothing is pending; it runs synchronously and its verdict is in hand before the round's wait would begin. Where a gate mixes both kinds, run skeptic first and let its findings join the same batch as the bot's — one combined evaluation per round, for the reason below. A gate with no bot entries has no wait at all, and a wait armed for one would never end.
