...
Fill `{{DELIVERY}}` in either brief exactly as for a reviewer ([`reviewer-brief.md`](reviewer-brief.md)), with "the classification" in place of "the blocks".
...
`asked` is not a decision, and it still counts as the most recent entry: a finding whose latest entry is `asked` has no decision on record, whatever an earlier entry says.
...
Every finding comes back, each with its bucket, any severity change, the prior thread URL where one applies, and — where it matched one of this skill's own threads — that comment's `databaseId` and whether the thread is resolved. The settled ones come back too, with the thread or record that decided them: they are reported as settled, not discarded. {{DELIVERY}} A bucketing that nobody receives sends stage 6 to its fallback, and a second run then opens a fresh thread beside each of its own earlier comments.
