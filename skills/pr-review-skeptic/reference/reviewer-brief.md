...
**Where the change edits prose, the unit of correctness is the claim rather than the file.** Documentation, marketing and release copy, user-facing strings, README and design text, and comments that state a fact all share one property code does not: the same claim is routinely written out in several places, in different words, with no compiler and no test tying the copies together. So a change that corrects a claim in one file characteristically leaves its siblings standing and now contradicting it — and the corrected file is the one in your range while the siblings are not, which is exactly why nobody sees it. The result is worse than the original, because a folder where one of five copies has been fixed reads as reviewed.

So for each claim the change alters — a fact about the product or a competitor, a version, a settings path, a number, a hedge added to an absolute, a claim retracted — go and find the other statements of it, by **content rather than by diff**: `git -C "{{REPO_PATH}}" grep -in` for a distinctive phrase, then for the subject on its own, since a sibling stating the same claim rarely uses the same wording. A claim can also wrap across a line break, which a phrase search cannot match, so search the subject in more than one spelling. Search the whole repository, not your slice. **Where you report that nothing of a kind exists, name what you searched for** — an absence can only be established by a search, and a reader can judge the claim only by knowing which one. A sibling that still asserts what this change retracted, or that contradicts what it now says, is a finding — `path` is the sibling, and `line: none` where that file is outside the pull request's diff. This is one of the findings this review most wants, and the sibling being untouched by the change is the reason it needs asking for rather than a reason to leave it.
