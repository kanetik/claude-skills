...
So for each claim the change alters — a fact about the product or a competitor, a version, a settings path, a number, a hedge added to an absolute, a claim retracted — go and find the other statements of it, by **content rather than by diff**: `git -C "{{REPO_PATH}}" grep -in` for a distinctive phrase, then for the subject on its own, since a sibling stating the same claim rarely uses the same wording. Search the whole repository, not your slice. A sibling that still asserts what this change retracted, or that contradicts what it now says, is a finding — `path` is the sibling, and `line: none` where that file is outside the pull request's diff. This is one of the findings this review most wants, and the sibling being untouched by the change is the reason it needs asking for rather than a reason to leave it.
