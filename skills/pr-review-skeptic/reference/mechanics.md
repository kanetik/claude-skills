...
- **Spell out what a `git` or `gh` call targets, and substitute `<tmp>` as a literal.** The guard reads each command statically and refuses one it cannot prove targets this worktree — naming another checkout does that, and so does a form it cannot resolve by reading, such as an argument taken from command output. Resolve `<tmp>` once and write the resolved path into each command rather than leaving the `$TMPDIR` expansion to be evaluated; the value does not change, only its spelling at the call site. Where a command is refused anyway, run its parts as separate commands rather than trying to talk the guard round.
...
#    whole-change=<head> -->` into a review body picks the scope: the sha passes the ancestry
...
#    past it too. The `whole-change` field is forgeable the same way and costs more: it is
...
# whole-change read on another's.
...
#   none  -> no whole-change read is established for this PR -- stage 3 sends the run to first-run scope
