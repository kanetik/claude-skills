...
**Say what you observed, not a cause you cannot see from here.** A uniform zero says the blocks did not arrive; it does not say the reviewers failed, and it is not by itself evidence of a whole-session condition. Reviews that ran to completion and had nowhere to deliver their result produce an identical signature — every unit at once, repeatable across re-dispatch — so the causes above are examples rather than the diagnosis. Check the dispatch contract (stage 4) before telling the user their subagent tooling is broken, and where a re-run is offered, offer it as a re-run that changes the dispatch: the same wiring re-invoked reproduces the same silence and costs another full set of reviews to do it. Posting here would put a review on the user's PR, under their account, notifying everyone, containing nothing; "0 of 4 units reviewed" in the coverage line does not redeem that.
