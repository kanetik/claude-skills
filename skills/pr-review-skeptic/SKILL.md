...
**A returning dispatch blocks you until the last reviewer is back, so write the staging lock's deadline before it, not during** ([`reference/mechanics.md`](reference/mechanics.md)). There is no turn between dispatching and collecting in which to re-touch anything, and a lock that ages out mid-pass tells the next run on this PR that the directory is a crashed run's leavings — which it clears, deleting the worktree these reviewers are reading. The same holds for the cross-check at stage 6, which blocks the same way.
...
**And one exit needs less than this step, not more: a failure inside stage 1 after the lock write but before the worktree exists** — a clone that fails, a fetch that cannot authenticate, a sha that will not resolve. There is nothing to remove and no ref to delete, but the lock is held, so that exit releases it and nothing else ([`reference/mechanics.md`](reference/mechanics.md)). Left behind it stops the next run on this PR for up to ninety minutes over a directory holding nothing but the lock — which under `pr-review-loop` pauses the loop and asks a person to wait out a run that is not running. None of it is anything to keep: left behind, the ref pins the PR's objects alive and the worktree shows up in the user's `git worktree list` and `git status` forever, at paths they were never told.
