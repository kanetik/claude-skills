...
**Where `max_reviewers` is smaller than that needs — `1`, which the "one reviewer" modifier sets and a repo may commit — the composition reviewer wins.** Spend the budget on it first and dispatch as many content reviewers as the remainder allows, none if that is none. It reads the whole change, so a run that keeps only it still covers every file; a run that kept only content reviewers would leave everything before `$LASTREVIEWED` unread while stage 7's coverage sentence claimed otherwise. Say in the coverage line how many content reviewers the cap displaced.
