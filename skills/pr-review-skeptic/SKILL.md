...
A `fixed` disposition is not settled either — it is a claim the code no longer has the problem, and testing that claim is exactly what a reviewer is for. **The composition reviewer is what keeps that true under delta scoping**, and it is why its deferral rule is conditional ([`reference/reviewer-brief.md`](reference/reviewer-brief.md)): `Fix-differently` and `Fix-broader` routinely land a repair in a different file from the finding, so the delta holds the repair and not the code the finding was about. No content reviewer reads that file — and the composition reviewer, which reads the whole change on every run, is told to leave a single-part defect to the reviewer holding that part **only where a reviewer is holding it**. On a later run nobody holds anything outside the delta, so those files are its own. Without that condition the `unfixed` bucket — the loudest signal this skill produces — could only ever fire when a fix happened to touch the file it was fixing.
