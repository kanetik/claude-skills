...
- **The prior review carries no coverage record at all** — an older run, from before the record existed — so `unreviewed-units` cannot be established for it. `commit_id` gives a plausible sha and says nothing about whether that run covered everything, and delta-scoping past a unit it left unreviewed would put a false claim in `{{PRIOR_REVIEW}}` for the rest of the PR's life. Unknown coverage is treated as incomplete coverage.
