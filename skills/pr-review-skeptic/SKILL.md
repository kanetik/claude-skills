...
- **The record carries no `composition` field** — an older record, from between versions — so how stale the last whole-change read is cannot be established. Unknown coverage of the whole change gets the same answer as unknown coverage of a unit. Coming here rather than merely adding a composition unit to a delta-scoped run is what keeps every run's reviewers on a single range, which the scope attribution `pr-review-loop` reads depends on; and it fires at most once per PR, because the record this run writes settles it for every run after.
...
**Read `composition` from the same record, and carry it forward.** It is the sha whose *whole change* a composition reviewer last read — `$REVIEWED` where one runs on this run, otherwise whatever the last record carried, and `none` where none has ever run on this PR (stage 7 writes it either way). Beyond the fallback above it triggers nothing on this run; the two cases under **Partition** decide the composition unit. It exists for the next run and for `pr-review-loop`, which gates convergence on it, so dropping it silently un-gates the merge. **A record carrying `reviewed` but no `composition` field is unknown, not `none`** — that is the fallback's case, and it is why the fallback is worth one repeated full pass.

**Otherwise** → a **later run**, and content units partition the **delta**, `$LASTREVIEWED..$REVIEWED` — the code written since the last review, which under `pr-review-loop` is the fix rounds, and which is the code with the least review behind it and the most accumulated confidence that it is fine. **Where the delta is empty, that decides both halves of the run: dispatch no content reviewers, and dispatch the composition reviewer over the whole change.** The empty delta says no new code was written; it does not say the whole change has been read against its current base. Two ways to reach it, and a whole-change read is what both of them call for. A collaborator clicks "Update branch" — `$BASE` moves to the new upstream tip, so the PR's own diff is now against a base nothing has ever been reviewed against. Or the loop has stopped moving HEAD and is one round from converging, which is the one moment the whole change *must* have been read (**Partition**, below). Stopping the run instead would hand `pr-review-loop` a round with no verdict at HEAD, which its reviewer can never be made happy by. Say in the coverage line that no new code was in scope. Where it fails to resolve for any other reason, fall back to the first-run scope and say the run did so.
...
| In the change but outside the delta | **only on a run that got a composition reviewer** (Partition, below) — its range is the whole change and its deferral is conditional on someone else holding that part, which outside the delta nobody does ([`reference/reviewer-brief.md`](reference/reviewer-brief.md)). On a later run with a non-empty delta there is none, so nobody | on those runs only |
...
The rows below the first are the residue: findings on files no reviewer's range contains this run. Do not paper over either by injecting those paths into a content unit — a content reviewer's range is the delta, its brief tells it to report what is in range, and a file with an empty range in its slice gets a contradiction rather than a re-read. **Name them in the coverage line instead**, and the two rows say different things:

- **Outside `$BASE...$REVIEWED`** — a file the PR never touched. No run re-tests these, on any cadence. Say the claims were not re-tested, full stop. Keeping tier 3 small is what keeps this small.
- **Inside the change but outside the delta, on a run with no composition reviewer.** Say those claims were not re-tested *this run*, and that the whole-change read the record's `composition` sha names is what covers them — which under `pr-review-loop` is guaranteed to happen at HEAD before the PR can converge. This row is the price of the cadence in Partition, and the honest version of it is a disclosure that names when the read does happen, not silence.
...
A **composition unit** takes the whole file list and the seams between the other units, and asks how the pieces behave together. Functions that are each correct alone and wrong in the arrangement production actually uses are invisible to every reviewer holding only one of them.

**Wherever it runs, its range is the whole change, `$BASE...$REVIEWED` — never the delta, on a first run or a tenth.** The defect class that survives every other kind of review is two changes that are each correct in isolation and wrong in combination, and across rounds that pairing is *made* by the fix rounds: round three's repair is right, round five's repair is right, and their product deletes something. Neither round's delta shows it. Only the whole diff does.

**A run gets a composition unit in exactly two cases:**

- **A first run whose partition yields more than one unit** — including a run sent here by any of the first-run fallbacks above, one of which is a record that cannot say when the whole change was last read. At a single unit there are no seams, one reviewer already holds the whole change, so there is none and nothing is lost.
- **A later run whose delta is empty.** No new code was written, so there are no content reviewers to dispatch and a whole-change read is the only thing this run can be.

**A later run with a non-empty delta gets no composition unit.** This reverses an earlier rule that gave one to every later run however few units the delta yielded, so it is worth saying what survived that reversal and what did not. The argument for it was that the seams it reads are seams of the *whole change*, which has them whether or not this round's delta does, and that the interactions it looks for are between code written rounds apart. **All of that is still true, and it is why the range above is never narrowed to the delta.** What it does not establish is a *cadence*. A pairing made by round three's and round five's repairs is in the whole diff at round five and still there at round ten; skipping the reads in between delays the finding, it does not lose it. The one thing that would be lost is the whole-change read of the *final* code — and that is a guarantee rather than a cadence. It is supplied by the empty-delta case above, which is what a converging PR's last round is, and by `pr-review-loop`'s convergence invariant, which will not report *converged* while the coverage record's `composition` sha is behind HEAD. So the whole change is still read cold before anyone merges. What stops is re-reading it on every round: on the run this was measured against it was the most expensive stage of each, mostly spent on rounds that had settled several rounds earlier.

**A later-run composition reviewer is therefore always the only reviewer on its run**, and its brief has to say so. Drop the "other reviewers are reading only what changed since `$LASTREVIEWED`" sentence from its `{{PRIOR_REVIEW}}` block ([`reference/reviewer-brief.md`](reference/reviewer-brief.md)) and say every part of the change is its to report. The slice replacement's deferral rule turns on that sentence, so leaving it in defers a single-part defect to a reviewer that never existed — on the one run where this is the only reviewer there is.

**`max_reviewers` counts the composition unit like any other reviewer**, and there is no budget contest to resolve: on a later run content units and a composition unit never co-occur, and an empty-delta run is one reviewer, which no sane cap displaces. On a first run the partition is sized so the content units plus the composition unit fit — larger units, never fewer files — and where the cap leaves room for a single reviewer the single-unit rule above governs, so that reviewer takes the whole change as a content unit and there is no composition unit. Nothing is lost there either: it already holds everything.
...
**Dispatch each reviewer in a form whose output comes back to you, and make the brief's closing instruction match it.** Concurrent does not mean detached: subagent tools generally offer both a call that hands the agent's report back to its caller and a fire-and-forget form whose output reaches you only if the agent explicitly sends it somewhere — and several returning calls issued together still run at the same time, so parallelism is not what the detached form buys. Take the returning form. Where only the detached one is available, the brief's closing line must say where to send the blocks instead of leaving them as a closing statement. A reviewer dispatched detached, told to hand its blocks back, performs the entire review and drops it on the floor, and what arrives at this stage is indistinguishable from a reviewer that never ran.

**A returning dispatch blocks you until the last reviewer is back, so touch the staging lock immediately before it, not during** ([`reference/mechanics.md`](reference/mechanics.md)). There is no turn between dispatching and collecting in which to touch anything, so that one write has to cover the whole pass — and a lock that ages out mid-pass tells the next run on this PR that the directory is a crashed run's leavings, which it clears, deleting the worktree these reviewers are reading. The same holds for the cross-check at stage 6, which blocks the same way.
...
Each reviewer returns `FINDING`, `NOTED` and `SOUND` blocks. **`NOTED` is not a finding and never becomes one**: it is something correct that nobody should have to act on — reachable only by ANDing unusual conditions, or costing a later reader mildly. It is not "a finding below the blocking severities": a `MEDIUM` and a `LOW` are findings, get threads, and get answered. It gets no thread, no severity, no bucket and no disposition, and it is carried to stage 7 for the body only ([`reference/reviewer-brief.md`](reference/reviewer-brief.md)). Promoting one to a `FINDING` to look thorough is what turns an honest review into a treadmill: each costs the author a round, and every round owes another review. A reviewer that returns none of the three has produced nothing to record — dispatch it again rather than recording silence as a clean unit, up to twice. **Note that silence says the blocks did not arrive, not that no review happened**, so a re-dispatch that repeats the first one exactly is only worth issuing once: if the second attempt is also silent, change how it is dispatched before the third, per the dispatch contract above. Three identical dispatches through wiring that delivers nothing cost three full reviews to learn what one already said. Still nothing after that, the unit is **unreviewed**: carry it forward, name it in the coverage line at stage 7 and in the report at stage 8, and keep going. An unreviewed unit is a hole in the review that the user has to know about; retrying it forever posts nothing at all.
...
- A later run's content reviewers saw only the code written since the last review (stage 3) → say so and name the commit: "no blocking findings in the changes since `<sha>`". A reader must not take a delta-scoped clean verdict for a fresh full-change one. **Then say when the whole change was last read**, from the `composition` sha the record carries: "the whole change was read by a composition reviewer at `<sha>`" — or, where that reviewer ran on *this* run, that it was read at `$REVIEWED`. Assert the second clause only from a sha you actually have; where the composition reviewer ran and returned nothing, or `composition` is `none`, the verdict covers the delta alone and must say so. This is the sentence that tells a reader how stale the whole-change read is, and under `pr-review-loop` a `composition` sha behind HEAD is also what holds convergence open, so it is never the last word on a PR that merges.
- Any `fixed` disposition since the last review that no reviewer's range covered (stage 3) → say those claims were not re-tested, and which kind they are: a path **outside `$BASE...$REVIEWED`** is re-tested by no run on any cadence, while one **inside the change but outside the delta** on a run with no composition reviewer waits for the whole-change read that the `composition` sha names.
...
**Nothing posts unless something was reviewed.** Where no unit returned a `FINDING`, `NOTED` or `SOUND` block — the subagent tool unavailable, rate-limited, or erroring, among other causes — the run has no review in it. **All three count, and the enumeration has to match stage 4's exactly**, because a whole-session dispatch failure produces no blocks at all while a reviewer that files two cornered notes and no `SOUND` has plainly reviewed its unit. Gate on two and that run reports "no review in it", names a cause that is false, discards the notes and posts nothing — so no coverage record is written, and under `pr-review-loop` the round reads as one where no unit got reviewed: skeptic is re-invoked on every wake, the run reaches `max_iterations`, and on a skeptic-only list excusing it reports *nobody reviewed this HEAD* on a PR that was reviewed and found clean.

**Say what you observed, not a cause you cannot see from here.** A uniform zero says the blocks did not arrive; it does not say the reviewers failed, and it is not by itself evidence of a whole-session condition. Reviews that ran to completion and had nowhere to deliver their result produce an identical signature — every unit at once, repeatable across re-dispatch — so the causes above are examples rather than the diagnosis. Check the dispatch contract (stage 4) before telling the user their subagent tooling is broken, and be exact about what a re-run buys, which depends on which of those it was. Unavailable or rate-limited, an unchanged re-run after a short wait is the remedy and costs nothing, because no reviewer ran. Undelivered, it reproduces the silence and spends another full set of reviews doing it — there, the re-run has to change the dispatch to be worth offering at all. Posting here would put a review on the user's PR, under their account, notifying everyone, containing nothing; "0 of 4 units reviewed" in the coverage line does not redeem that.
...
- **Comments** — one per finding, carrying its severity, the defect, its consequence, and the fix. `unfixed` findings say how many rounds have already touched that code; `re-raised` ones link the thread — or cite the dispositions comment — where the concern was dismissed. Every comment ends with the marker line `<!-- pr-review-skeptic -->`, which is how a later run recognises its own work: these reviews post under the user's account and are otherwise indistinguishable from a hand-written one.
  ...
  **Tier-3 findings get their scope too.** They have no comment and no marker, so the body's tier-3 heading carries `scope=whole` or `scope=delta` per finding, in the same words. Without it a single body-listed finding leaves the loop unable to attribute *any* of the round's findings, and the share it needs is voided by the one tier this brief actively solicits — findings on files the change never touched.

  **On a later run, add a scope line immediately before it** — `<!-- pr-review-skeptic: scope=whole -->` for a finding from a whole-change reviewer, `<!-- pr-review-skeptic: scope=delta -->` for one from a delta-scoped content reviewer. Omit the line entirely on a first run, where every reviewer had the whole change. **A run's reviewers all share one range** (stage 3: content reviewers and a composition reviewer never co-occur on a later run), so the value is uniform across a round and there is no merged-from-both case to break a tie on. Emit it per finding all the same — that is what `pr-review-loop` parses, and a per-round claim it would have to reconstruct is a parser change for no gain. **It is a separate line and the plain marker stays exactly as it is** — every matcher in both skills tests `contains("<!-- pr-review-skeptic -->")` for that exact string, so folding an attribute into it would make this skill stop recognising its own work: no thread matching, no `$LASTREVIEWED`, every run a first run. It exists for one consumer: `pr-review-loop` reports what share of a round's findings landed in code the loop itself wrote, and that share is meaningless unless it is taken over reviewers who could have found something else — a delta-scoped reviewer on a loop-driven PR is reading the loop's fix commits by construction. Without the attribute the loop cannot compute it and cannot honestly say it did.
- **A summary body** — the verdict; the coverage (the scope the content reviewers were given — the whole change, or the changes since a named commit — plus files reviewed, units, how many reviewers, whether the cap forced larger units, and any unit left unreviewed); the tier-3 findings under their own heading, at full severity, marked as having no thread; **the `NOTED` items under a heading of their own**, plainly marked as things nobody needs to act on or respond to, so they are not dispositioned or counted as work — and kept clearly apart from the findings, which include every `MEDIUM` and `LOW` and do need an answer; the `settled` list with the thread or dispositions-comment record that decided each; and, when a run was narrowed by a modifier, what it did not cover. It ends with the same marker line the comments carry — invisible in the rendered body, and the only way a later run can tell whether a review it could not confirm actually landed. A clean verdict has no comments at all, so without it there would be nothing on the PR to recognise.

  **Immediately before that marker, write the coverage record**, on its own line and in exactly this form:

  ```
  <!-- pr-review-skeptic: reviewed=<$REVIEWED> unreviewed-units=<n> composition=<sha|none> -->
  ```

  `<$REVIEWED>` is the sha the blind reviewers actually read. **`<n>` is the number of content units that got no per-unit read this run** — those stage 4 carried forward after a failed dispatch. `0` on a normal run, and `0` on an empty-delta run, where no content units existed to go unread. This is the field's one definition; stage 3 reads it and does not restate it.

  **`composition` is the sha whose whole change a composition reviewer last read**: `$REVIEWED` where one ran on this run and returned blocks, and otherwise the value the last record carried, unchanged — `none` only where no composition reviewer has ever run on this PR. Carrying it forward is the whole point of the field: most later runs have no composition unit (stage 3), so a field that reset to `none` on those runs would say the whole change had never been read on a PR where it had, and one that wrote `$REVIEWED` on them would claim a read that did not happen. The first stalls `pr-review-loop` short of convergence forever; the second lets it converge and merge with a whole-change read several rounds stale, which is the coverage claim this field exists to keep honest.

  All three fields exist because the alternatives are wrong in ways nothing surfaces: the review's `commit_id` is the *current* head rather than the reviewed one on the force-push path, and a coverage hole recorded only in English prose cannot stop the next run — or the loop — from scoping past it. Write the record on every posted review, clean verdict included.
...
Remove the staged worktree, delete `refs/prskeptic/<num>`, and drop the temp clone where the run made one ([`reference/mechanics.md`](reference/mechanics.md)). **Every exit after stage 1 comes through here** — a posted review, a shown preview, and the deliberate stops at stages 2 and 3 alike. **One exit does not, and it is the one that would do the most damage by coming through: the `LIVE-RUN-SUSPECTED` stop inside stage 1** ([`reference/mechanics.md`](reference/mechanics.md)). That run staged nothing — it stopped precisely because `<tmp>` belongs to another run — so there is nothing of its own here to remove, and `rm -rf "<tmp>"` would delete a live run's worktree, payload files and lock while its reviewers are mid-read.

**And one exit needs a narrower version of this step: a failure inside stage 1 after the lock write but before the worktree exists** — a clone that fails, a fetch that cannot authenticate, a sha that will not resolve. There is no worktree yet, but the lock is held and **the ref exists from the first fetch onward** ([`reference/mechanics.md`](reference/mechanics.md)), so that exit clears `<tmp>` and deletes the ref where the fetch ran. Skip it and the lock stops the next run on this PR for up to ninety minutes — which under `pr-review-loop` pauses the loop and asks a person to wait out a run that is not running — while the ref pins the PR's objects alive in a checkout nobody will think to look in.
