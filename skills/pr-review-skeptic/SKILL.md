...
- **Tell it that it is the only reviewer.** Where the cap leaves zero content reviewers, drop the "other reviewers are reading only what changed since `$LASTREVIEWED`" sentence from its `{{PRIOR_REVIEW}}` block ([`reference/reviewer-brief.md`](reference/reviewer-brief.md)) and say every part of the change is its to report. The conditional deferral then resolves the right way on its own.
