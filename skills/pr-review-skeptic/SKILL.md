...
A `fixed` disposition is not settled either — it is a claim the code no longer has the problem, and testing that claim is exactly what a reviewer is for. **Delta scoping would quietly make that untestable, so it takes one addition:** add the paths those findings named to the content units alongside the delta. `Fix-differently` and `Fix-broader` routinely land the repair in a different file from the finding, so the delta contains the repair and not the code the finding was about; without this the `unfixed` bucket — the loudest signal this skill produces — can only ever fire when a fix happened to touch the file it was fixing. Those paths are the one class of already-reviewed file a run has a positive reason to re-read.
